...
class HybridKeywordExtractor:
    ...
    def __init__(
        ...
        # AI 추출기 초기화
        self.ai_extractor = AIKeywordExtractor(ai_client) if self.enable_ai else None

        logger.info(f"하이브리드 추출기 초기화: NLP={enable_nlp}, AI={self.enable_ai}")
    ...
    async def extract(
        ...
        if len(combined) >= min_keywords:
            logger.info(f"충분한 키워드 발견 ({len(combined)}개), AI 생략")
        ...
        if self.enable_ai and self.ai_extractor:
            ...
            ai_result = await self.ai_extractor.extract(
                query, min_confidence=self.ai_min_confidence
            )
            ai_keywords = ai_result.get("keywords", [])
            ai_confidence = ai_result.get("confidence", 0.0)
            sources["ai"] = ai_keywords
            logger.debug(
                f"Stage 3 (AI): {len(ai_keywords)}개 (신뢰도: {ai_confidence:.2f})"
            )
    ...
    def _map_categories(self, keywords: list[str]) -> list[str]:
        ...
        for (
            category_name,
            category_obj,
        ) in self.keyword_config.get_all_categories().items():
